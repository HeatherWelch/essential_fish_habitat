...
#####################################################################################################################
################################## Netcdf with no time dimension  ###################################################
#####################################################################################################################
df2 <- subset(df, time == 42154) # Just one time value
# Set dimensions
lons <- unique(df2$lon)
lats <- unique(df2$lat)

# Reshape to wider matrix
toShapeW <- pivot_wider(df2, id_cols = c("lon"), values_from = "pred", names_from = "lat") 
toShapeW$lon <- NULL
suppressWarnings(dimnames(toShapeW) <- list(lon = lons, lat = lats))
