...
climdir=glue("{outdir}/La_nina");dir.create(climdir)
...
for(i in 1:length(seasons)){
  ...
  pred_map=ggplot()+
    ...
    ggtitle(glue("{species} 1981-2022 {season_names[i]} La Nina"))+
    theme(legend.position = "bottom",
          legend.key.width = unit(1, 'cm'))

  png(glue("{climdir}/{species}_1981-2022_laNina_{season_names[i]}.png"),width=11,height=11,units='cm',res=400,type = "cairo")
  par(ps=10)
  par(mar=c(0,0,0,0))
  par(cex=1)
  print({pred_map})
  ...
  ncfname <-glue("{climdir}/{species}_1981-2022_laNina_{season_names[i]}.nc")
